...
> **当前版本**：OpenClaw v2026.3.11 | **主力 AI**：WorkBuddy（腾讯云运维）| **接入方式**：Tailscale HTTPS
...
├── README.md                          # 本文件
│
├── workspace/                         # ⭐ 龙虾规则文件体系（虚拟机 ~/.openclaw/workspace/ 镜像）
│   ├── AGENTS.md                      # 启动规程（v4，含L0硬拒绝，最高优先级）
│   ├── AI_RULES.md                    # 安全规则 v2.4.0-lobster
│   ├── SOUL.md                        # 龙虾的性格与原则
│   ├── USER.md                        # 主人档案（脱敏）
│   ├── SELF_KNOWLEDGE.md              # 龙虾的自我认知与能力说明
│   ├── TOOLS.md                       # 工具使用规范（v3，含MCP工具）
│   ├── KNOWLEDGE_BASE.md              # 结构化状态快照（v1.1）
│   └── skills/                        # 技能文件
│       ├── rules-loader.md            # 合规门禁技能
│       ├── task-planner.md            # 任务规划技能
│       ├── workbuddy-dna.md           # WorkBuddy 工作方式传承
│       ├── github-sync.md             # GitHub日志同步技能（v3）
│       └── feishu-file-reader.md      # 飞书文件解析技能
│
├── feishu-bot/                        # 飞书机器人相关
...
## 🔌 MCP 工具配置（2026-03-15 新增）

OpenClaw 已接入 4 个 MCP Server，具备与 WorkBuddy 相近的工具能力：

| MCP Server | 对应 WorkBuddy 工具 | 安装方式 | 版本 |
|---|---|---|---|
| `filesystem` (@modelcontextprotocol/server-filesystem) | read_file / write_to_file / list_dir / search_file | npm global | 2026.1.14 |
| `fetch` (mcp-server-fetch) | web_fetch（网页抓取→Markdown） | pip | 2025.4.7 |
| `websearch` (websearch-mcp) | web_search（网络搜索，免 API key） | npm global | 1.0.3 |
| `desktop-commander` (@wonderwhy-er/desktop-commander) | execute_command（命令执行增强） | npm global | 0.2.38 |

**安装位置**：
- npm 包：`/home/xzy0626/.local/lib/node_modules/`
- Python 包：`/home/xzy0626/.local/bin/mcp-server-fetch`
- 配置在：`~/.openclaw/openclaw.json` → `mcpServers` 字段
- 备份：`~/.openclaw/openclaw.json.bak.mcp-20260315_112420`

**后续改进方向**：
- `@playwright/mcp`：完整浏览器自动化（可处理 JS 渲染页面）
- Tavily/Brave API：替换 websearch-mcp，搜索质量更高
